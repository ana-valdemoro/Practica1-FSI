...
#Devuelve una lista cerrada. Si yo quisiera que funcionara como una FIFO tendria que implementar mi propio Append,pop,
# extend
def Stack():
...
#Append inserta un item al final de la lista
...
#Todas estas lineas de codigo casi que se podrian sustituri por A.pop(0). Pero si tu haces esto muchas muchas veces
#como puede ser una búsqueda. Decae el rendimiento. Por eso estas lineas.
...
#POdemos duplicar el QUEUE Y MODIFICAR UNAS COSAS
#Extende- añadir una lista más y en ese momento reordenar todas la lista. Para que cuando se haga el pop se saque el de
#el de menor cost_path
#Búsqueda en anchura y reordena por parth-cost
class NuevaQueue(Queue):
    """A First-In-First-Out Queue."""
    def __init__(self):
        self.A = []
        self.start = 0
    ...
    def __len__(self):
        return len(self.A) - self.start
#Items es una lista con los nodos hijos a añadir a la lista abierta
#extend inserta los items por el final
    """"Se reordena en el método extend porque por sentido natural cada vez que insertas ordenas"""
    def extend(self, items):
        self.A.extend(items)
        #ahora deberiamos ordenar self por path cost
...
class HeuristicCostQueue(Queue):
    """A First-In-First-Out Queue."""
    def __init__(self,p):
        self.A = []
        self.start = 0 #utilizamos self para referirnos a la isntancia del objto actual
        self.problem = p #Esto funciona porque asocias cada instancia de problema a su cola
